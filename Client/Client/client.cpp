...
public:
	...
	UIManager(sf::RenderWindow* g_window) {
		...
		m_exptexture[0] = new sf::Texture;
		m_exptexture[0]->loadFromFile("Resource/UI/grayhp.png");
		m_expImg[0].setTexture(*m_exptexture[0]);
		m_expImg[0].setTextureRect(sf::IntRect(0, 0, 512, 512));
		m_expImg[0].setScale(1.5f, 0.06f);
		m_expImg[0].setPosition(200, w_window_height - 80);

		m_exptexture[1] = new sf::Texture;
		m_exptexture[1]->loadFromFile("Resource/UI/greenhp.png");
		m_expImg[1].setTexture(*m_exptexture[1]);
		m_expImg[1].setTextureRect(sf::IntRect(0, 0, 512, 512));
		m_expImg[1].setScale(1.5f, 0.06f);
		m_expImg[1].setPosition(200, w_window_height- 80);


		m_exptexture[2] = new sf::Texture;
		m_exptexture[2]->loadFromFile("Resource/UI/expui.png");
		m_expImg[2].setTexture(*m_exptexture[2]);
	...
	void Draw() {
		...
		if (bChattingmode) {
			...
			std::wstring expstring = L"경험치:                     ";
			float exp = Owner->m_exp;
			float maxexp = Owner->m_level * 100;
			float ratioexp = 1.5f * exp / maxexp;
			m_expImg[1].setScale(ratioexp, 0.06f);
			expstring += std::to_wstring((int)exp);
			expstring += L"/";
			expstring += std::to_wstring((int)maxexp);
	

			m_pwindow->draw(m_expImg[0]);
			m_pwindow->draw(m_expImg[1]);


			exp_display.setString(expstring);
			exp_display.setFont(g_font);
			chatcontent_display.setStyle(sf::Text::Italic);
			exp_display.setFillColor(sf::Color(255, 255, 255));
			exp_display.setPosition(200, w_window_height - 80);
			m_pwindow->draw(exp_display);
